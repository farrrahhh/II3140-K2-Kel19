...
const Belajar4 = () => {
  ...
  useEffect(() => {
    // Animasi khusus untuk level 4 yang aktif
    if (level === 4) {
      Animated.loop(
        Animated.sequence([
  ...
  const renderLevelIcons = () => {
    return levelIcons.map((item) => {
      const isActive = item.id <= 4; // Level 1, 2, 3, dan 4 aktif
      const isAnimating = item.id === 4; // Animasi hanya untuk level 4
          ...
          <View style={styles.mantraContent}>
            ...
            <View style={styles.mantraRight}>
              ...
              <Text style={styles.mantraText}>
                "Great things are achieved by taking small, consistent steps forward."
              </Text>
            </View>
          </View>
        </View>
