...
const Belajar3 = () => {
  ...
  useEffect(() => {
    // Animasi khusus untuk level 3 yang aktif
    if (level === 3) {
      Animated.loop(
        Animated.sequence([
  ...
  const renderLevelIcons = () => {
    return levelIcons.map((item) => {
      const isActive = item.id <= 3; // Level 1, 2, dan 3 aktif
      const isAnimating = item.id === 3; // Animasi hanya untuk level 3
          ...
          <View style={styles.mantraContent}>
            ...
            <View style={styles.mantraRight}>
              ...
              <Text style={styles.mantraText}>
                "Keep climbing. Each step forward is a step toward success."
              </Text>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  outerCircle: {
    ...
    alignItems: 'center',
    borderWidth: 3,
    borderColor: '#ffd09b', // Warna kuning untuk level aktif (1, 2, dan 3)
  },
  levelCircle: {
    width: 100,
    height: 100,
